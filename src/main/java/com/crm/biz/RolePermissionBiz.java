...
public class RolePermissionBiz {
	...
	private static final Logger LOGGER = Logger
			.getLogger(RolePermissionBiz.class);
	...
	public Message<Integer> deleteRolePermission(Integer id, String operatorId) {
		...
		try {
			RolePermissionMapper rolePermissionMapper = mapperFactory
					.getMapperForMaster(RolePermissionMapper.class);
			int result = rolePermissionMapper.logicalDeleteByPrimaryKey(id,
	...
	private boolean validateForCreate(RolePermission rolePermission,
			Message<?> message) {
	...
	public Message<Integer> updateRolePermission(RolePermission rolePermission) {
		...
		try {
			...
			RolePermission rolePermissionLast = rolePermissionMapper
					.selectByPrimaryKeyForUpdate(rolePermission.getId());
			...
			int result = rolePermissionMapper
					.updateByPrimaryKey(rolePermission);
			mapperFactory.commitTransaction();
			message.setData(result);
	...
	private boolean validateForUpdate(RolePermission rolePermission,
			Message<?> message) {
	...
	public Message<Integer> changeStatus(Integer id, Byte status,
		...
		try {
			...
			int result = rolePermissionMapper.changeStatus(id, status,
					operatorId, System.currentTimeMillis());
			mapperFactory.commitTransaction();
			message.setData(result);
	...
	public Message<RolePermission> getRolePermission(Integer id) {
		...
		try {
			RolePermissionMapper rolePermissionMapper = mapperFactory
					.getMapperForSlave(RolePermissionMapper.class);
	...
	public Message<QueryResult<RolePermission>> queryRolePermission(
		...
		try {
			...
			PageHelper.startPage(rolePermissionQuery.getPageIndex(),
					rolePermissionQuery.getPageSize());
			List<RolePermission> rolePermissionList = rolePermissionMapper
					.queryRolePermission(rolePermissionQuery);
			PageInfo<RolePermission> pageInfo = new PageInfo<RolePermission>(
					rolePermissionList);
	...
	public Message<Integer> savePermissionBYroleid(Integer[] resourceids,
			Integer roleid, Integer create_id) {
		Message<Integer> message = new Message<Integer>();
		try {
			RolePermissionMapper rolePermissionMapper = mapperFactory
					.getMapperForMaster(RolePermissionMapper.class);
			int result = rolePermissionMapper.logicalDeleteByRoleid(roleid);

			for (int i = 0; i < resourceids.length; i++) {
				RolePermission rolePermission = new RolePermission();
				rolePermission.setCreateUserid(create_id);
				rolePermission.setResourceid(resourceids[i]);
	...
	public Message<List<RolePermission>> getPermissionByRoleid(Integer roleid) {
